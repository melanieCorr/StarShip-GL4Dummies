...
static void init(void) {
  ...
  _quad = gl4dgGenQuadf();
  initText(&_textTexId, 
	   "Réalisé par Mélanie De Jesus Correia, étudiante en Licence 2 informatique à l'université Paris 8\n\n\n"
	   "Pour ce projet, je me suis inspirée de la saga Star Wars,"
	   " car ce sont des films que j'apprécie énormément et n'ayant pas eu"
	   " réellement d'idée jusque là, j'ai choisi de m'inspirer de choses que"
	   " j'aime.\n\n\nJe vous souhaite un bon visionnage en espérant que ma démo"
	   " vous plaira. Et bien sûr, que la Force soit avec vous !\n\n\n"
	   " Sources : code programme : Mr. Farès Belhadj\n\n"
	   " Musiques : https://www.youtube.com/watch?v=v7QJjGyUbak"
...
static void draw(void) {
  ...
  if(t0 < 0.0f)
    t0 = SDL_GetTicks();
  t = (SDL_GetTicks() - t0) / 2000.0f, d = -1.5f /* du retard pour commencer en bas */ + 0.25f /* vitesse */ * t;
...
static void initText(GLuint * ptId, const char * text) {
  ...
  if( !(font = TTF_OpenFont("STJEDISE.ttf", 128)) ) {
